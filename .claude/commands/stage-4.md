...
- 누락된 핵심 섹션이 없는가
- KPI가 구체적이고 측정 가능한가
- 기능 요구사항마다 수용 기준(acceptance criteria)이 구체적이고 검증 가능한가
...
| 조건 | 판정 |
|------|------|
| Critical/Major 없음 | **통과** → 5단계 채택 |
| Critical/Major가 모두 문서 결함 | **3단계 회귀** (수정 지침 첨부) |
| Critical/Major에 아이디어 결함 포함 (수정 가능) | **2단계 회귀** |
| 아이디어 결함이 근본적·수정 불가 | **탈락** (사용자 동의 필수) |

- **통과** → "5단계: `/stage-5 NNN-아이디어명 채택`" 안내
- **3단계 회귀** → 리뷰 보고서의 수정 지침을 사용자에게 제시한 뒤 "3단계: `/stage-3 NNN-아이디어명`" 안내
- **2단계 회귀** → 근본적 사업성/기술 전제 변경 필요 시 "2단계: `/stage-2 NNN-아이디어명`" 안내
- **탈락** → "5단계: `/stage-5 NNN-아이디어명 탈락`" 안내 (사용자 동의 필수)
