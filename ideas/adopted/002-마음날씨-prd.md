...
CREATE TABLE public.notification_settings (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT chk_max_notifications
    CHECK (TRUE)  -- 앱 레벨에서 user_id당 최대 3개 제한
);
    ...
    'emotion_distribution', (
      ...
      FROM (
        ...
        FROM public.emotion_logs el,
             UNNEST(el.emotion_tag_ids) AS tag_id
  ...
  SELECT jsonb_build_object(
    ...
    'activity_emotion_correlation', (
      ...
      FROM (
        ...
        FROM public.emotion_logs el,
             UNNEST(el.activity_tag_ids) AS act_id,
             UNNEST(el.emotion_tag_ids) AS emo_id
  ...
  RETURN (
    ...
    FROM (
      SELECT
        ...
        COUNT(*) AS log_count,
        -- 그 날의 대표 감정 (가장 빈번한 태그)
        (
          SELECT et.color
          FROM UNNEST(array_agg(el2.emotion_tag_ids)) AS all_tags(tag_arr),
               UNNEST(tag_arr) AS tag_id
          JOIN public.emotion_tags et ON et.id = tag_id
...
BEGIN
  ...
  IF v_current_count > 3 THEN
    -- 롤백하지 않고, 클라이언트에서 사전 체크하므로 여기까지 오면 경계값
...
| 6 | 핵심 DB Function의 SQL 본문이 완성되었는가 | **충족** | 섹션 7-5-5: 6개 RPC(get_weekly_summary, get_monthly_pattern_report, get_selfcare_recommendations, get_calendar_summary, delete_user_account, increment_daily_card_usage) 전체 SQL 본문 작성 |
